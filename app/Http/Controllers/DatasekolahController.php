...
class DatasekolahController extends Controller
{
    ...
    public function getIndex(Request $request) {
        $sekolah = DB::table('tblsekolah')->orderBy('idsekolah', 'DESC')->get();
        ...
        //                 $btn = '<a href="/datasekolah/show/'.base64_encode($row->idsekolah).'"><i class="fas fa-eye" style="font-size:15px;color:green;"></i></a>
        //                 &nbsp;&nbsp;<a href="/datasekolah/edit/'.base64_encode($row->npsn).'"><i class="fas fa-edit" style="font-size:15px;color:#009ef7;"></i></a>
        //                 &nbsp;&nbsp;
        //                 ';
    ...
    public function getEdit($id) {
        $provinsi = DB::table('tblwilayah')->where('kode', '18')->get();
        $sekolah = DB::table('tblsekolah')->where('npsn', base64_decode($id))->get();
    ...
    public function getShow($id) {
        $provinsi = DB::table('tblwilayah')->where('kode', '18')->get();
        $sekolah = DB::table('tblsekolah')->where('idsekolah', base64_decode($id))->get();
    ...
    public function update(Request $request)
    {
        ...
        $batas = request()->validate([
         'logosekolah'  => 'image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);
  
        if ($files = $request->file('logosekolah')) {
            $datasekolah = DB::table('tblsekolah')->where('idsekolah',$request->input('id'))->get();
            $this->delete_image($datasekolah[0]->logo);
             
            //store file into document folder
            $image = $request->file('logosekolah');
            ...
            $data = array(
                ...
                'perpus' => $request->post('perpus'),
                'reg_date' => Carbon::now()->toDateTimeString(),
                'emailsekolah' => $request->post('email_sekolah'),
                'logo' => $new_name
            );
        }else{
            $data = array(
    ...
    {
        if ($img && file_exists(public_path('logo/'.$img)))
            return unlink(public_path('logo/'.$img));
        // return $img;
    }
