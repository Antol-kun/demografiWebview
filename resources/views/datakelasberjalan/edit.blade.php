...
<a href="/datakelasberjalan" class="btn btn-sm btn-primary">Kembali ke {{$testVariable}}</a>
@endsection
    ...
    <div id="kt_content_container" class="container">
        ...
        <div class="card">
            ...
            <div class="card-body pt-0">
                ...
                <div class="row mb-5">
                    <!--begin::Col-->
                    <input type="hidden" name="id" value="{{$data->idkelasberjalan}}"/>
                    <div class="col-md-6 fv-row">
                        ...
                        <option value="{{$data->idsettingtahun}}" selected>{{$data->tahunakademik}} / {{$data->semester}}</option>
                        @foreach($thnajaran as $thn)
                              <option value="{{$thn->idsettingtahun}}">{{$thn->tahunakademik}} / {{$thn->semester}}</option>
                        @endforeach
                        </select>
                        <!--end::Input-->
                    </div>
                    ...
                    <div class="col-md-6 fv-row">
                        ...
                        <!--begin::Input-->
                         <select name="rombel" class="form-select form-select-solid">
                          ...
                          <option value="{{$data->idkelompokkls}}" selected>{{$data->kode_kelompok}}</option>
                          @foreach($kelompok as $klp)
                            <option value="{{$klp->idkelompokkls}}">{{$klp->kode_kelompok}}</option>
                          @endforeach
                        </select>
                        <!--end::Input-->
                    </div>
                ...
                <div class="row mb-5">
                        ...
                        <!--begin::Select-->
                               <select name="kelas" class="form-select form-select-solid">
                                  <option value="" disabled selected>Pilih Kelas...</option>
                                   <option value="{{$data->idruangkelas}}" selected>{{$data->kode_ruang}}</option>
                                   @foreach($ruang as $r)
                                   <option value="{{$r->idruang}}">{{$r->kode_ruang}}</option>
                                   @endforeach
                                </select>
                               <!--end::Select-->
                        <!--end::Input-->
                    ...
                    <div class="col-md-6 fv-row">
                        ...
                        <select name="walikelas" class="form-select form-select-solid">
                            ...
                            <option value="{{$data->idguru}}" selected>{{$data->Nip}} - {{$data->Nama}}</option>
                            @foreach($guru as $gr)
        ...
        $.ajaxSetup({
            ...
            $('#saveBtn').click(function (e) {
                ...
                $.ajax({
                  ...
                  url: "/datakelasberjalan/update",
                  type: "POST",
                  dataType: 'json',
                  success: function (data) {
                    if(data.message == 'match'){
                        Swal.fire("Perhatian !", "Data kelompok kelas berjalan sudah terdaftar !", "warning");
                        document.getElementById("saveBtn").disabled = false;
                        $('#saveBtn').html('Simpan');
                    }else if(data.message == 'success'){
                        Swal.fire("Berhasil", "Data kelompok kelas berjalan berhasil diubah", "success");
